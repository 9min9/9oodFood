...
import goodfood.exception.user.NotFoundLoginTokenException;
...
public class JwtTokenProvider {
    ...
    public TokenInfo generateToken(Authentication authentication) {
        ...
        String refreshToken = Jwts.builder().setExpiration(new Date(now + 86400000))
                .signWith(key, SignatureAlgorithm.HS256)
                .compact();
        return TokenInfo.builder().grantType("Bearer").accessToken(accessToken).refreshToken(refreshToken).userRole(authorities).build();
    }

    // JWT Token을 복호화 하여 Token의 정보를 꺼내는 메서드
    public Authentication getAuthentication(String accessToken){
        //Token 복호화
        Claims claims = parseClaims(accessToken);

        if(claims.get("auth") == null) {
            throw new NotFoundLoginTokenException("권한 정보가 없는 토큰입니다.");
        }
