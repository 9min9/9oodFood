function createPageTitle() {
    ...
    if(urlLocation == "seoul") {
        titleData = "서울 ";
    } else if (urlLocation == "gyeonggi") {
        titleData = "경기 ";
    } else if (urlLocation == "etc") {
        titleData = "기타 지역 ";
    }
...
function createBreadcrumbItem() {
    ...
    var categoryId = 'sub-title-category';
    var categoryClass = 'breadcrumb-item active';

    categoryItem.setAttribute('id', categoryId);
    categoryItem.setAttribute('class', categoryClass);
...
//    var id = "forum-" +jsonData.content[i].id;
...
//    var cardId = "store-card" + i;
    ...
    storeCard.appendChild(imageZone);   // child(img src)
    storeCard.appendChild(titleZone); // child (titleLabel)
    storeCard.appendChild(dataZone); // child( address, info, mood)
